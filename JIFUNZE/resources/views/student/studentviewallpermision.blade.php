        ...
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          ...
          <li class="nav-item">
            ...
            <ul class="nav nav-treeview">
              ...
              <li class="nav-item">
                <a href="viewrepliedperm" class="nav-link">
                  <i class="far fa-circle nav-icon"></i>
                  <p>Replyed Permisions</p>
                  <span class="right badge badge-danger">{{$student->permission->where('status',)->count()}}  </span>
                </a>
              </li>
                ...
                <div class="table-responsive">
                      ...
                      @if ($student->permission->count() > 0)
                        @foreach ($student->permission as $permission)
                            <tr>
                              ...
                              <td>
                                <button data-bs-target="#viewperm{{$permission->id}}" data-bs-toggle="modal" class="btn btn-primary btn-sm"> <i class="bi bi-eye"></i> </button>
                  ...
                  <div class="text-count pt-3 pb-5">
                      <div class="float-right fw-bold" style="color: #a5a4a4;">
                        All Permissions: {{ $student->permission->count() }}
                      </div>
                  </div>
                </div>
            </div>
...
@foreach ($student->permission as $permission)
  <div class="modal fade" id="viewperm{{$permission->id}}" tabindex="-1"
    aria-labelledby="viewperm{{$permission->id}}Label" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header" style="display:flex; align-items:center;">
              <div class="text" style="display:flex; flex-direction:column;">
                <div class="h5">Student | View Permision</div>
                <div class="badge badge-success" style="width: fit-content;">{{$permission->created_at}}</div>
              </div>
              <button class="float-right btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
               <div class="row">
                  <div class="col-md-5">
                   <label for="studentname" id="studentname" class="form-label">You <span class="text-muted">(Student)</span></label>
                   <input type="text" name="studentname" id="studentname" class="form-control" value="{{$permission->student->firstname. " ".$permission->student->lastname }}" readonly>
                  </div>
                  <div class="col-md-2">
                      <div class="icon" style="width: 100%; height:100%; display:flex; align-items:center; justify-content:center;">
                        <i class="bi bi-arrow-right"></i>
                      </div>
                  </div>
                  <div class="col-md-5"> 
                    <label for="studentname" id="studentname" class="form-label">To <span class="text-muted">(Lecture)</span></label>
                    <input type="text" name="lecturename" id="lecturename" class="form-control" value="{{$permission->lecture->firstname. " ".$permission->lecture->lastname }}" readonly>
                   </div></div>
               </div>

                <div class="contentview px-3">
                    ...
                    <div class="mb-3">
                      <label for="reply" id="reply" class="form-label">Reply</label>
                      <div class="text p-3" style="width: auto; background-color: #e9e9e9;">
                        @if (empty($permission->reply->reply))
                          <em>Not Replayed</em>
                        @else
                          {{ optional($permission->reply)->reply }} 
                        @endif
